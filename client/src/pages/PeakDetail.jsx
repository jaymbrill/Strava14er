...
function SummitHistoryCard({ summit, onDelete }) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await axios.delete(`/api/activities/summit/${summit.id}`, { withCredentials: true });
      onDelete(summit.id);
    } catch {
      setDeleting(false);
    }
...
export default function PeakDetail() {
  ...
  useEffect(() => {
    axios
      .get(`/api/fourteeners/${id}/history`, { withCredentials: true })
    ...
    <div className="min-h-screen bg-gradient-to-b from-co-peak via-[#0f1e35] to-[#0a1628]">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className={`rounded-3xl border p-6 sm:p-8 mb-6 relative overflow-hidden
          ${completed
            ? 'bg-gradient-to-br from-co-gold/10 to-white/5 border-co-gold/30'
        ...
        >
          ...
          <div className="relative z-10">
            <div className="flex flex-wrap items-start gap-3 mb-2">
              ...
              {peak.rank && (
                <span className="text-sm px-3 py-1 rounded-full border border-white/20 text-white/60 bg-white/5">
                  #{peak.rank} in CO
                </span>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-6 text-sm">
              <div>
                ...
                <div className="text-2xl font-bold text-co-gold font-mono">{peak.elevation.toLocaleString()}′</div>
              </div>
              <div>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            {history.length > 1 && (
              <span className="text-xs text-co-gold bg-co-gold/10 px-3 py-1 rounded-full border border-co-gold/20">
          ...
          {history.length === 0 ? (
            <div className="text-center py-16 text-white/40">
              ...
              <p className="text-sm max-w-xs mx-auto mb-5">
                Sync your Strava activities from the dashboard, or add this summit manually.
              </p>
        ...
        <div className="mt-4 text-center">
          <a
            href={`https://www.14ers.com/routemain.php?peak=${encodeURIComponent(peak.name)}`}
            target="_blank"
            rel="noopener noreferrer"
            className="text-sm text-white/40 hover:text-co-gold transition-colors"
          >
            View routes & beta on 14ers.com →
          </a>
        </div>
      </main>
    </div>
